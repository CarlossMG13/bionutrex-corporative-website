...
export function HomeDataProvider({ children }: HomeDataProviderProps) {
  ...
  const applyPendingChanges = useCallback((pendingChanges: PendingChange[]) => {
    setBaseSections(currentBase => {
      let updatedSections = [...currentBase];
    ...
    setBaseSliders(currentBase => {
      let updatedSliders = [...currentBase];
      pendingChanges.forEach(change => {
        if (change.type === 'slider') {
          switch (change.action) {
            case 'update':
  ...
  // Sync sections/sliders when base data changes
  useEffect(() => {
  ...
  const getSectionByKey = useCallback((key: string): HomeSection | undefined => {
    return sections.find(section => section.sectionKey === key && section.active);
  }, [sections]);

  const getActiveSliders = useCallback((): Slider[] => {
    return sliders.filter(slider => slider.active).sort((a, b) => a.order - b.order);
  }, [sliders]);

  const getSortedSections = useCallback((): HomeSection[] => {
    return sections
